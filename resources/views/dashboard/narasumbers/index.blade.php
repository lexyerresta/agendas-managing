...
@section('container')
    <div class="d-flex justify-content-between flex-wrap flex-md-nowrap align-items-center pt-3 pb-2 mb-3 border-bottom">
        <h1 class="h2">List Permintaan Narasumber BNNP Bali</h1>
    </div>
    @can('admin')
    <a href="/dashboard/narasumbers-softdelete" class="btn btn-danger mb-2 me-3">Recycle Bin</a>
            ...
            <tr>
                ...
                <td>
                    @if (!$narasumber->tanggal_approve)
                        <a href="/dashboard/narasumbers/{{ $narasumber->id }}/edit" class="badge bg-warning"><span data-feather="edit"></span></a>
                        @can('admin')
                            <form action="/dashboard/narasumbers/{{ $narasumber->id }}" method="post" class="d-inline">
                                @method('delete')
                                @csrf
                        ...
                        @endcan
                        <form action="{{ route('dashboard.narasumbers.approve', $narasumber->id) }}" method="POST" class="d-inline">
                            @csrf
                            @method('PUT')
